...
import Attempt from "./../src/db/attempt.js";
...
export default class Bench {
	...
	async goto({ details }) {
		const timeout = details?.timeout || config.timeout;
		this.#url = details?.url;
		this.#wait = details?.wait || config.wait;

		if (!this.#url) {
			throw new Error('No URL passed for the action "goto"!');
		...
		try {
			...
			await this.#page.goto(this.#url, { timeout, waitUntil: "networkidle2" });
		} catch (error) {
		...
		if (knock) {
			try {
				this.#logger.info(`Trying actually going to URL "${this.#url}" without networkidle2.`);
				await this.#page.goto(this.#url, { timeout, waitUntil: "load" });
			} catch (error) {
				this.#logger.error(`Could not goto URL: ${error}!`);
				return { success: false, error: error.message };
			}
		...
		try {
			if (knock) {
				this.#logger.info(`Waiting just a bit longer, "${this.#wait * 3}ms".`);
				await this.#page.waitForTimeout(this.#wait * 3);
			} else {
		...
		if (scrape) {
			...
			for (const channel of this.#channels) {
				...
				this.#asset = await this.#saveAsset({
					...
					screenshotLocation: `${baseTargetPath}-screenshot.jpg`,
